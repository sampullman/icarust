...
- **World bounds are `WORLD_WIDTH × WORLD_HEIGHT` (3200 × 540).** The visible viewport (`VIEW_WIDTH × VIEW_HEIGHT` = 1280 × 540 in `crates/client/src/lib.rs`) is smaller than the world so the camera scrolls horizontally instead of wrapping at the screen edge. The X axis is still toroidal in the sim — entities that fly off the right reappear on the left — and `Camera::world_x_offsets_for` returns up to three world copies near the seam so wrap-around stays seamless visually. Resizing the window only changes letterboxing.
- **Player HP and damage.** Players have `PLAYER_MAX_HP` (5) hit points. Enemy bullets deal one point; rocks, ramming, and terrain crashes still instant-kill. HP regenerates +1 every `PLAYER_REGEN_INTERVAL` after `PLAYER_REGEN_DELAY` damage-free seconds. The client renders an HP bar in the HUD, brown smoke from damaged ships (`render::particles::DamageSmoker`), and a one-shot puff on `GameEvent::PlayerDamaged`.
- **Procedural entity art.** `render::entities::EntityMeshes` builds one ggez `Mesh` per kind at startup; `mesh_for_kind` picks the right (mesh, tint) pair at draw time. Player thrust spawns flame particles via `render::particles::ThrustEmitter`. Background sky + clouds live in `render::sky::Sky`, seeded so all clients see the same skyline.
